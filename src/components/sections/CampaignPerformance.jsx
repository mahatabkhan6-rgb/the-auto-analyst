...
export default function CampaignPerformance({ data }) {
  ...
  if (!data.available) {
    return (
      <SectionCard icon={Megaphone} title="Campaign Analysis" number={6}>
        <p className="text-sm text-slate-500 leading-relaxed">{data.note || "Campaign-level analysis is unavailable — no campaign column was detected in the dataset."}</p>
      </SectionCard>
    );
  }
  ...
  return (
    <SectionCard
      ...
      subtitle="The AI's read of campaign patterns — table available as drill-down"
      number={6}
    >
      {insights.length > 0 && (
        <div className="mb-4">
      ...
      <p className="text-sm text-slate-500 leading-relaxed mb-3">{data.note}</p>
      <button onClick={() => setShow((v) => !v)} className="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-slate-900">
        {show ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
        {show ? "Hide" : "Show"} campaign detail ({campaigns.length} rows)
      </button>
      {show && (
            ...
            <tbody className="divide-y divide-slate-100">
              {campaigns.map((c, i) => (
                <tr key={i} className="text-slate-700">
                  <td className="px-3 py-2 whitespace-nowrap">{c.channel}</td>
                  <td className="px-3 py-2 whitespace-nowrap">{c.campaign}</td>
                  <td className="px-3 py-2 text-right font-mono whitespace-nowrap">{fmtMoney(c.spend)}</td>
                  <td className="px-3 py-2 text-right font-mono whitespace-nowrap">{fmtNum(c.signups)}</td>
                  <td className="px-3 py-2 text-right font-mono whitespace-nowrap">{fmtNum(c.converted)}</td>
                  <td className="px-3 py-2 text-right font-mono whitespace-nowrap">{fmtMoney(c.revenue)}</td>
                  <td className="px-3 py-2 text-right font-mono whitespace-nowrap">{c.cac === null ? "—" : `$${Math.round(c.cac).toLocaleString()}`}</td>
                  <td className="px-3 py-2 text-right font-mono whitespace-nowrap">{c.roas === null ? "—" : `${c.roas.toFixed(2)}x`}</td>
                </tr>
              ))}
            </tbody>
          </table>
